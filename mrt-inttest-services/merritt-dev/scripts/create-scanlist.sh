...
case $MERRITT_ECS in
  ...
  "ecs-ephemeral")
    ...
    ;;
  "docker")
    aws s3 ls --profile minio-docker --recursive s3://${BUCKET7777}/ | cut -c 32- > /tmp/scanlist
    aws s3 cp --profile minio-docker /tmp/scanlist s3://${BUCKET8888}/scanlist/7777.log
    ;;
  *)
